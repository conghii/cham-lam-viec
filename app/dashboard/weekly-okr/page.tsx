...
import { Loader2, ChevronLeft, ChevronRight, Calendar, CheckCircle2, Plus, Trophy, AlertTriangle, Lightbulb, History, Minus } from "lucide-react";
...
function TimeNavigation({
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-xl shadow-sm">
                <div className="flex items-center gap-2">
                    <Button variant="outline" size="icon" onClick={onPrev}>
                        <ChevronLeft className="h-4 w-4" />
                    </Button>
                    <div className="text-center px-4 min-w-[200px]">
                        <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t("week_label")} {weekNum}</h2>
...
function BigObjectiveCard({
    ...
    if (!objective || !objective.content && !readOnly) {
        // Empty State
        return (
            <Card className="h-48 border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex flex-col items-center justify-center text-slate-400 hover:border-blue-400 hover:text-blue-500 transition-colors cursor-pointer group"
                onClick={() => {
            ...
            >
                ...
                <span className="font-medium">{t("set_key_result")} #{index + 1}</span>
            </Card>
        );
    }
    ...
    return (
        <Card className={cn(
            "min-h-[14rem] transition-all relative overflow-hidden group flex flex-col justify-between",
            isCompleted
            ...
            <div className="absolute top-0 right-0 p-3 z-10">
                ...
                >
                    <CheckCircle2 className={cn("h-6 w-6", isCompleted ? "fill-current" : "")} />
                </button>
            </div>
            ...
            <CardContent className="h-full flex flex-col p-6 gap-4">
                <div>
                    <div className="flex justify-between items-center mb-3 pr-8">
                        ...
                        {!readOnly && (
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                {!hasMetric && (
                                    <Button variant="ghost" size="sm" onClick={() => onUpdate({ target: 1, current: 0, unit: "" })} className="h-6 text-xs text-slate-400 hover:text-blue-500 p-0 hover:bg-transparent px-2">
                                        <Plus className="h-3 w-3 mr-1" /> {t("add_metric")}
                                    </Button>
                                )}
                                {hasMetric && (
                                    <Button variant="ghost" size="sm" onClick={() => onUpdate({ target: undefined, current: undefined, unit: undefined })} className="h-6 text-xs text-slate-400 hover:text-red-500 p-0 hover:bg-transparent px-2">
                                        {t("remove_metric")}
                                    </Button>
                                )}
                    ...
                    <input
                        ...
                        className={cn(
                            "w-full bg-transparent border-none text-xl md:text-2xl font-bold placeholder:text-slate-300 focus:ring-0 p-0 resize-none mb-2",
                ...
                {/* Metric Section - Super Clean Focused Row */}
                {hasMetric && (
                    <div className="mt-auto pt-2 space-y-4">
                        <div className="flex items-center justify-center gap-2 px-[40px] group/metric">
                            {/* Minus Button */}
                            <Button
                                disabled={readOnly}
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 rounded-full text-slate-300 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all shrink-0"
                            ...
                            <div className="flex items-center gap-3 px-2">
                                ...
                                <div className="flex items-center gap-3 min-w-0">
                                    <Input
                                        type="number"
                                        className="h-7 w-9 text-center bg-transparent border-none focus:ring-0 p-0 text-lg font-semibold text-slate-500 dark:text-slate-400 hover:text-blue-500 transition-colors [&::-webkit-inner-spin-button]:appearance-none"
                                        value={objective.target || 0}
                                        disabled={readOnly}
                                        onChange={(e) => handleMetricUpdate('target', Number(e.target.value))}
                                    />
                                    <Input
                                        className="h-7 w-auto min-w-[40px] bg-transparent border-none focus:ring-0 p-0 text-sm font-medium text-slate-400 text-left hover:text-slate-600 transition-colors truncate"
                                        placeholder="unit"
    ...
    return (
        <div className="h-full p-4 md:p-8 max-w-6xl mx-auto space-y-8 relative">
            <div className="flex justify-between items-start">
                <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">{t("weekly_pulse_title")}</h1>
