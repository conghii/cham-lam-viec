...
export function DeepDiveFlow({ onComplete, onCancel, initialData }: DeepDiveFlowProps) {
    ...
    return (
        <div className="w-full max-w-2xl mx-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl animate-in fade-in zoom-in-95 duration-300">
            {/* Steps Indicator */}
                    ...
                    <div key={s} className="flex items-center">
                        <div className={cn(
                            ...
                            step === s ? "bg-indigo-600 text-white scale-110 shadow-lg shadow-indigo-500/30" :
                                step > s ? "bg-green-500 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-400"
                        )}>
                        ...
                        {s < 4 && (
                            <div className={cn(
                                ...
                                step > s ? "bg-green-500" : "bg-slate-100 dark:bg-slate-800"
                            )} />
                        )}
                    </div>
                ))}
            ...
            <div className="min-h-[300px]">
                    ...
                    <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
                        <div className="text-center mb-6">
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center justify-center gap-2">
                                <Target className="w-6 h-6 text-indigo-500" /> {t("dd_defining_success")}
                            </h3>
                            <p className="text-slate-500 dark:text-slate-400">{t("dd_defining_success_desc")}</p>
                        </div>

                        <div className="space-y-4">
                            <Label>{t("dd_key_results")}</Label>
                            ...
                            <Button variant="outline" size="sm" onClick={addKeyResult} className="gap-2 text-indigo-500 border-dashed border-indigo-200 dark:border-indigo-900">
                                <Plus className="w-4 h-4" /> {t("dd_add_kr")}
                            </Button>
                        </div>

                        <div className="space-y-2">
                            <Label>{t("dd_domain_area")}</Label>
                ...
                {step === 2 && (
                    <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
                        <div className="text-center mb-6">
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center justify-center gap-2">
                                <BarChart className="w-6 h-6 text-indigo-500" /> {t("dd_current_state")}
                            </h3>
                            <p className="text-slate-500 dark:text-slate-400">{t("dd_current_state_desc")}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {LEVELS.map(level => (
                                ...
                                >
                                    ...
                                    <div className="font-bold text-slate-900 dark:text-white">{t(`dd_level_${level.id.toLowerCase()}` as any) || level.label}</div>
                        ...
                        <div className="space-y-2">
                            <Label>{t("dd_past_experience")}</Label>
                            <Textarea
                                ...
                                className="min-h-[100px] bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 focus:bg-white dark:focus:bg-slate-900 transition-colors"
                            />
                        </div>
                    </div>
                )}
                ...
                {step === 3 && (
                    <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
                        <div className="text-center mb-6">
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center justify-center gap-2">
                                <Clock className="w-6 h-6 text-indigo-500" /> {t("dd_reality_check")}
                            </h3>
                            <p className="text-slate-500 dark:text-slate-400">{t("dd_reality_check_desc")}</p>
                        </div>

                        <div className="space-y-4">
                            <Label>{t("dd_availability")}</Label>
                                ...
                                {DAYS.map(day => (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            !data.weeklyHours[day] ? "bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-transparent text-slate-400" :
                                                data.weeklyHours[day] === 2 ? "bg-indigo-100 dark:bg-indigo-900/40 border-indigo-200 dark:border-indigo-800/50 text-indigo-800 dark:text-indigo-200" :
                                                    "bg-indigo-500 dark:bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-500/20"
                                        )}
                                    >
                                        <span className="text-xs font-bold uppercase">{t(day.toLowerCase() as any)}</span>
                ...
                {step === 4 && (
                    <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
                        <div className="text-center mb-6">
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center justify-center gap-2">
                                <Brain className="w-6 h-6 text-indigo-500" /> {t("dd_brain_dump")}
                            </h3>
                            <p className="text-slate-500 dark:text-slate-400">{t("dd_brain_dump_desc")}</p>
                        </div>
                        ...
                        <div className="relative">
                            <Textarea
                                ...
                                placeholder={t("dd_brain_dump_placeholder")}
                                className="min-h-[200px] p-6 text-lg bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 resize-none shadow-inner focus:bg-white dark:focus:bg-slate-900 transition-colors"
                            />
                            <div className="absolute bottom-4 right-4 flex gap-2">
                                <Button variant="secondary" size="icon" className="rounded-full h-10 w-10 shadow-md bg-white dark:bg-slate-800" title="Voice Input (Coming Soon)">
                                    <Mic className="w-5 h-5 text-slate-500" />
                                </Button>
                                <Button variant="secondary" size="icon" className="rounded-full h-10 w-10 shadow-md bg-white dark:bg-slate-800" title="Attach File (Coming Soon)">
            ...
            {/* Navigation Actions */}
            <div className="flex justify-between mt-8 pt-6 border-t border-slate-100 dark:border-slate-800/50">
                <Button
                    variant="ghost"
                    onClick={step === 1 ? onCancel : prevStep}
                    className="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
                >
                    {step === 1 ? t("dd_cancel") : t("dd_back")}
                </Button>

                {step < 4 ? (
                    <Button
                        onClick={nextStep}
                        className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-100 px-8 rounded-full shadow-lg shadow-slate-200 dark:shadow-none"
                    >
                        {t("dd_next_step")} <ChevronRight className="w-4 h-4 ml-2" />
                    </Button>
                ) : (
                    <Button
                    ...
                    >
                        {t("dd_complete")} <Sparkles className="w-4 h-4 ml-2" />
                    </Button>
                )}
            </div>
        </div>
