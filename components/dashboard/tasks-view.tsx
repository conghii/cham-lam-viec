...
import { Trash2, Plus, Calendar as CalendarIcon, LayoutList, Kanban, Grid2X2, Loader2, ArrowRight, GripVertical, Pencil, MessageSquare, Send, CheckCircle2, Circle, MoreVertical, Edit2, X, Check, Target, User, Tag as TagIcon, Settings2, Palette } from "lucide-react";
import {
    Select,
    SelectContent,
    SelectItem,
...
const defaultTags: Tag[] = [
    { id: 'general', label: 'General', color: 'bg-slate-50 text-slate-600 border-slate-100' },
    { id: 'coding', label: 'Coding', color: 'bg-blue-50 text-blue-600 border-blue-100' },
    { id: 'learning', label: 'Learning', color: 'bg-emerald-50 text-emerald-600 border-emerald-100' },
    { id: 'writing', label: 'Writing', color: 'bg-purple-50 text-purple-600 border-purple-100' },
    { id: 'design', label: 'Design', color: 'bg-pink-50 text-pink-600 border-pink-100' },
];

const presetColors = [
    { label: 'Slate', value: 'bg-slate-50 text-slate-600 border-slate-100' },
    { label: 'Blue', value: 'bg-blue-50 text-blue-600 border-blue-100' },
    { label: 'Green', value: 'bg-emerald-50 text-emerald-600 border-emerald-100' },
...
function TaskCard({ task, compact = false, columns, onMove, dragHandleProps, members = [], groups = [], role = 'member', orgId, tags = [], onEditTag, onDeleteTag, onCreateTag }: TaskCardProps) {
    ...
    return (
        <>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                        ...
                        <div className="flex items-center gap-1">
                            {(() => {
                                ...
                                const currentTag = availableTags.find(t => t.id === (task.tag || "general")) || availableTags.find(t => t.label.toLowerCase() === (task.tag || "general").toLowerCase()) || availableTags[0];
                                return (
                                    <Badge variant="outline" className={cn("rounded-md border px-1.5 font-normal capitalize h-5", currentTag?.color)}>
            ...
            <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
                <DialogContent showCloseButton={false} className="!w-[60vw] !max-w-[60vw] h-[80vh] overflow-hidden flex flex-col p-0 gap-0">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        {isEditMode ? (
                                            ...
                                            <div className="flex items-center h-8">
                                                {(() => {
                                                    const availableTags = tags.length > 0 ? tags : defaultTags;
                                                    const currentTag = availableTags.find(t => t.id === (tag || "general")) || availableTags.find(t => t.label.toLowerCase() === (tag || "general").toLowerCase()) || availableTags[0];
                                                    return (
                                                        <Badge variant="outline" className={cn("font-normal capitalize", currentTag?.color)}>
                                ...
                                <Dialog open={isTagManagerOpen} onOpenChange={setIsTagManagerOpen}>
                                    <DialogContent>
                                        <DialogHeader>
                                            ...
                                            <DialogDescription>Create, edit, and remove tags for your organization.</DialogDescription>
                                        </DialogHeader>

                                        <div className="space-y-4 py-4">
                                            <div className="flex gap-2 items-end">
                                                <div className="space-y-2 flex-1">
                                                    <Label>{editingTag ? "Edit Tag Name" : "New Tag Name"}</Label>
                                                    <Input
                                                        value={newTagName}
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <Popover>
                                                        <PopoverTrigger asChild>
                                                            <Button variant="outline" className={cn("w-9 px-0", newTagColor)}>
                                                                <div className={cn("h-4 w-4 rounded-full", newTagColor.split(" ")[0].replace("bg-", "bg-"))} />
                                                            </Button>
                                                        </PopoverTrigger>
                                                        <PopoverContent className="w-[180px] p-2">
                                                            <div className="grid grid-cols-4 gap-2">
                                                                {presetColors.map((c) => (
                                                                    <div
                                                                        key={c.value}
                                                                        className={cn("h-6 w-6 rounded-full cursor-pointer border hover:scale-110 transition-transform", c.value, newTagColor === c.value && "ring-2 ring-primary ring-offset-2")}
                                                ...
                                                <Button
                                                    onClick={async () => {
                                                        ...
                                                        if (editingTag) {
                                                            await updateTagInOrganization(orgId, editingTag.id, newTagName, newTagColor);
                                                            setEditingTag(null);
                                                        } else {
                                                            await addTagToOrganization(orgId, newTagName, newTagColor);
                                                        }
                                                        setNewTagName("");
                                                        setNewTagColor(presetColors[0].value);
                                                    }}
                                            ...
                                            <ScrollArea className="h-[200px] border rounded-md p-2">
                                                <div className="space-y-2">
                                                    {orgTags.length === 0 ? (
                                                        <p className="text-sm text-center text-muted-foreground py-8">No custom tags yet.</p>
                                                    ) : (
                                                        orgTags.map(tag => (
                                                            <div key={tag.id} className="flex items-center justify-between p-2 rounded-lg border bg-card hover:bg-muted/30 group">
                                                                <Badge variant="outline" className={cn("font-normal capitalize", tag.color)}>
                                                                    {tag.label}
                                                                </Badge>
                                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                    ...
                                                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => {
                                                                        if (confirm("Delete this tag?")) {
                                                                            if (orgId) deleteTagFromOrganization(orgId, tag.id);
                                                                        }
                                                                    }}>
                                                                        <Trash2 className="h-3.5 w-3.5" />
                                                                    </Button>
